"""
HES Trade Agent - Market Regime Intelligence.
    ...
    def classify(self,candles:Iterable[Candle])->List[MarketRegime]:
        ...
        for (symbol,timeframe),group in groups.items():
            ...
            if len(ordered)<self.min_candles or ordered[0].open==0:
                label="insufficient_data"; net_return=0.0
            else:
                net_return=(ordered[-1].close-ordered[0].open)/ordered[0].open
                label="uptrend" if net_return>=self.trend_threshold else "downtrend" if net_return<=-self.trend_threshold else "range"
            average_range=sum(c.high-c.low for c in ordered)/len(ordered) if ordered else 0.0
            results.append(MarketRegime(symbol,timeframe,label,len(ordered),net_return,average_range))

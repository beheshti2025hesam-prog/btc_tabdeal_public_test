...
class IntegrityAudit:
    ...
    def audit(self, trades: Iterable[CanonicalTrade]) -> dict:
        ...
        return {"total_trades": sum(report["total_trades"] for report in reports.values()), "groups": reports}

    @staticmethod
    def _audit_group(trades: list[CanonicalTrade]) -> dict:
        by_timestamp = sorted(trades, key=lambda trade: trade.timestamp)
        with_sequence = [trade for trade in trades if trade.sequence is not None]
        by_sequence = sorted(with_sequence, key=lambda trade: trade.sequence)
        sequences = [trade.sequence for trade in with_sequence]
        unique_sequences = set(sequences)
        duplicate_sequence_count = len(sequences) - len(unique_sequences)
        ...
        sequence_gap_count = len(gap_sizes)
        sequence_gap_total = sum(gap_sizes)
        largest_sequence_gap = max(gap_sizes, default=0)
        backward_sequence_count = sum(
            1 for previous, current in zip(by_timestamp, by_timestamp[1:])
            if previous.sequence is not None and current.sequence is not None and current.sequence < previous.sequence
        )
        timestamp_backward_count = sum(
            1 for previous, current in zip(by_sequence, by_sequence[1:])
            if current.timestamp < previous.timestamp
        )
        unique_event_ids = {trade.event_id for trade in trades}
        return {
            "total_trades": len(trades),
            "unique_event_ids": len(unique_event_ids),
            "duplicate_event_id_count": len(trades) - len(unique_event_ids),
            ...
            "largest_sequence_gap": largest_sequence_gap,
            "min_sequence": min(unique_sequences) if unique_sequences else None,
            "max_sequence": max(unique_sequences) if unique_sequences else None,

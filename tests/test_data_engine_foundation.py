...
class DataEngineFoundationTests(unittest.TestCase):
    ...
    def test_validator_rejects_invalid_numeric_fields(self):
        row = raw_row(sequence="not-an-int")
        row["price"] = "bad"
        row["amount"] = "bad"
    ...
    def test_normalizer_is_timezone_aware_and_utc(self):
    ...
    def test_integrity_detects_backward_sequence_by_timestamp(self):
        normalizer = RawDataNormalizer()
        trades = [
            normalizer.normalize_row(raw_row(sequence="200", updated="2026-09-24T00:00:00Z")),
            normalizer.normalize_row(raw_row(sequence="199", updated="2026-09-24T00:00:01Z")),
        ]
        report = IntegrityAudit().audit(trades)["groups"][("tabdeal", "BTC_USDT")]
        self.assertEqual(report["backward_sequence_count"], 1)

    def test_engine_deduplicates_and_orders_without_mutating_raw(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            active = root / "trades.csv"
            archive = root / "archive"
            ...
            self.assertEqual(engine.last_quality_report.validation["valid_rows"], 4)
            self.assertEqual(engine.last_quality_report.metadata["deduplicated_trades"], 3)

    def test_engine_skips_invalid_rows_and_reports_them(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            active = root / "trades.csv"
            archive = root / "archive"
            archive.mkdir()
            header = ["symbol", "price", "amount", "side", "updated", "sequence"]
            rows = [raw_row(sequence="100"), raw_row(sequence="bad")]
            with active.open("w", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(f, fieldnames=header)
                writer.writeheader()
                writer.writerows(rows)
            ...
            self.assertEqual(engine.last_quality_report.validation["invalid_rows"], 1)

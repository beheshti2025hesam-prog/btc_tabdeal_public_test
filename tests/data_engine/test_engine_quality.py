...
def test_engine_deduplicates_duplicate_event():
    ...
    engine = DataEngine(reader=DuplicateReader())
    ...
    assert report.metadata["deduplicated_trades"] == 1
    assert report.has_integrity_issues is True

    group = report.integrity[("tabdeal", "BTC_USDT")]

    assert group["duplicate_event_id_count"] == 1
